...
async function ziskejSeznamSouboru() {
  try {
    // Přidáváme timestamp, aby GitHub API neposílalo stará data (cache)
...
async function nactiProdukty(kategorie) {
  ...
  if (container) container.innerHTML = "Cargando productos...";

  for (const file of seznam) {
    try {
      const resp = await fetch(`/${PRODUCT_PATH}/${file}`);
      const data = await resp.json();
      // Oprava: převedeme na malá písmena pro přesnější porovnání
    ...
    cont.innerHTML += `
      <div class="produkt-card">
        ...
        <div style="display: flex; gap: 10px; margin-top: 10px; flex-wrap: wrap;">
          ...
          <button class="produkt-back-btn" onclick="history.back()" style="background:#666; color:white; border:none; padding:8px 12px; border-radius:4px; cursor:pointer;">VOLVER</button>
        </div>
      </div>`;
  });
}
